...
const FoxyKombat = () => {
  ...
  const [boostCost, setBoostCost] = useState(1000);
  const [isZoomed, setIsZoomed] = useState(false);
  const [bubbles, setBubbles] = useState([]);

  useEffect(() => {
    setBoostCost(1000 * level);
  }, [level]);
  ...
  return (
    <div className="bg-gradient-to-b from-black to-orange-900 min-h-screen text-white font-sans">
      <div className="max-w-sm mx-auto px-4 py-8">
        ...
        <div 
          className={`bg-blue-600 rounded-full p-4 mb-6 cursor-pointer relative overflow-hidden transition-transform duration-200 ${isZoomed ? 'scale-105' : ''}`} 
          onClick={handleFoxClick}
        >
          <img 
            src="https://firebasestorage.googleapis.com/v0/b/unify-v3-copy.appspot.com/o/j5agu4qpoi-1%3A234?alt=media&token=a7330b2c-75e5-4e95-a5b1-afd0bed2104e" 
            alt="Foxy character" 
            className="w-full"
          />
          {bubbles.map(bubble => (
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            className={`bg-orange-500 text-white px-4 py-2 rounded-full ${coinCount < boostCost ? 'opacity-50 cursor-not-allowed' : ''}`}
            onClick={handleBoost}
            disabled={coinCount < boostCost}
          >
            Boost
          </button>
        </div>
        ...
        <nav className="flex justify-between bg-gray-800 rounded-full p-2">
          <button className="p-2">
            <img src="/placeholder.svg?height=24&width=24" alt="Exchange" className="w-6 h-6" />
          </button>
          <button className="p-2">
